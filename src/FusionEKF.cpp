...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    MatrixXd P_in = MatrixXd(4, 4); // same for both
    MatrixXd F_in = MatrixXd(4, 4); // same
    MatrixXd Q_in = MatrixXd(4, 4); // needs to be set

    // one time initialization
    Q_in << noise_ax / 4., 0, noise_ax / 2., 0, 0, noise_ay / 4., 0,
        noise_ay / 2., noise_ax / 2., 0, noise_ax, 0, 0, noise_ay / 2., 0,
        noise_ay;
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      ekf_.Init(ekf_.x_, P_in, F_in, Hj_, R_radar_, Q_in);
